...
- **Creditreform-Relevanz-Score** (`cr_relevance`, 1–10): Klassifikator vergibt pro Artikel zusätzlich zur LLM-Konfidenz einen geschäftsbezogenen Relevanz-Score nach festen Ankern (10 = CH-Gesetzesänderung Bonität/Inkasso/DSG; 8 = CH-Konkurs benannte Firma; 3 = internationale Sanktionsmeldungen ohne CH-Bezug; 1 = Politik/Sport/Kultur). Gespeichert in `articles.cr_relevance` (Schema v7).
- **Relevance-based topic ordering**: Email topics sorted by average `cr_relevance` (highest first); Email-Subject zeigt den Artikel mit globalem Max-`cr_relevance` (Tie-Break: `credit_impact` negative → neutral → positive, dann `confidence`). Legacy-Artikel ohne `cr_relevance` werden als 0 gewertet (Variant C, kein Backfill).
